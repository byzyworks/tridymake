...
export const checkPaths = async (paths) => {
    for (let pathname of paths) {
        pathname = path.join(mapped.APP.ROOT, pathname);

        await fs.promises.access(pathname);
    }
};

export const safeImport = async (absolute, relative, modname) => {
    const thisdir  = path.join(absolute, relative);
    let   thispath = path.join(absolute, relative, modname + '.js');
    if (!thispath.startsWith(thisdir)) {
        throw new Error(`Directory traversal outside the parser sub-directory is not allowed.`);
    }

    // Because import() doesn't like paths on Windows.
    thispath = path.relative(absolute, thispath);
    thispath = thispath.replace(/\\/g, '/');

    const mod = await import(thispath);

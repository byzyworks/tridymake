...
echo -n "> Would you like to make $bin/tridy.sh callable from everywhere? [Y/n] ";
read make_shortcut;
echo;
if [ "$make_shortcut" = 'Y' ]; then
    ...
    mkdir -p "$newbin";
    ln -sf "$bin/tridy.sh" "$newbin/tridy";
fi

echo "> Running a test...";
if ! bin/tridy.sh make --file "src/tests/hello-world/test.tri"; then
